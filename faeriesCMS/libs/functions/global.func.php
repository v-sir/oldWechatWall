...
function sys_auth($string, $operation = 'ENCODE', $key = '', $expiry = 0) {
	$ckey_length = 4;
	$key = md5($key != '' ? $key : faeriesCMS_base::load_config('system', 'auth_key'));
...
function L($language = 'no_language',$pars = array(), $modules = '') {
	...
	if(defined('IN_ADMIN')) {
		...
		$lang = faeriesCMS_base::load_config('system','lang');
	}
	if(!$LANG) {
		require_once FC_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.'system.lang.php';
		if(defined('IN_ADMIN')) require_once FC_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.'system_menu.lang.php';
		if(file_exists(FC_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.ROUTE_M.'.lang.php')) require_once FC_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.ROUTE_M.'.lang.php';
	}
	if(!empty($modules)) {
		$modules = explode(',',$modules);
		foreach($modules AS $m) {
			if(!isset($LANG_MODULES[$m])) require_once FC_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.$m.'.lang.php';
		}
	}
...
function template($module = 'content', $template = 'index', $style = '') {
	...
	$template_cache = faeriesCMS_base::load_sys_class('template_cache');
	$compiledtplfile = faeriesCMS_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.php';
	if(file_exists(FC_PATH.'templates'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html')) {
		if(!file_exists($compiledtplfile) || (@filemtime(FC_PATH.'templates'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html') > @filemtime($compiledtplfile))) {
			$template_cache->template_compile($module, $template, $style);
		}
	} else {
		$compiledtplfile = faeriesCMS_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.php';
		if(!file_exists($compiledtplfile) || (file_exists(FC_PATH.'templates'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html') && filemtime(FC_PATH.'templates'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html') > filemtime($compiledtplfile))) {
			$template_cache->template_compile($module, $template, 'default');
		} elseif (!file_exists(FC_PATH.'templates'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html')) {
...
function my_error_handler($errno, $errstr, $errfile, $errline) {
	if($errno==8) return '';
	$errfile = str_replace(faeriesCMS_PATH,'',$errfile);
	if(faeriesCMS_base::load_config('system','errorlog')) {
...
function setcache($name, $data, $filepath='', $type='file', $config='', $timeout=0) {
	...
	faeriesCMS_base::load_sys_class('cache_factory','',0);
	if($config) {
		$cacheconfig = faeriesCMS_base::load_config('cache');
...
function getcacheinfo($name, $filepath='', $type='file', $config='') {
	if(!preg_match("/^[a-zA-Z0-9_-]+$/", $name)) return false;
	if($filepath!="" && !preg_match("/^[a-zA-Z0-9_-]+$/", $filepath)) return false;
	faeriesCMS_base::load_sys_class('cache_factory');
	if($config) {
		$cacheconfig = faeriesCMS_base::load_config('cache');
		$cache = cache_factory::get_instance($cacheconfig)->get_cache($config);
	} else {
...
if (!function_exists('iconv')) {
	function iconv($in_charset, $out_charset, $str) {
		...
		if (function_exists('mb_convert_encoding')) {
			...
			faeriesCMS_base::load_sys_func('iconv');
			$in_charset = strtoupper($in_charset);
			$out_charset = strtoupper($out_charset);
...
function show_ad($siteid, $id) {
	...
	$p = faeriesCMS_base::load_model('poster_model');
...
function get_nickname($userid='', $field='') {
	...
	if(is_numeric($userid)) {
		$member_db = faeriesCMS_base::load_model('member_model');
...
function get_memberavatar($uid, $is_userid='', $size='30') {
	if($is_userid) {
		$db = faeriesCMS_base::load_model('member_model');
	...
	faeriesCMS_base::load_app_class('client', 'member', 0);
	define('APPID', faeriesCMS_base::load_config('system', 'phpsso_appid'));
	$phpsso_api_url = faeriesCMS_base::load_config('system', 'phpsso_api_url');
	$phpsso_auth_key = faeriesCMS_base::load_config('system', 'phpsso_auth_key');
...
function thumb($imgurl, $width = 100, $height = 100 ,$autocut = 1, $smallpic = 'nopic.gif') {
	global $image;
	$upload_url = faeriesCMS_base::load_config('system','upload_url');
	$upload_path = faeriesCMS_base::load_config('system','upload_path');
	...
	if(!is_object($image)) {
		faeriesCMS_base::load_sys_class('images','','0');
		$image = new image(1,0);
	}
...
function go($catid,$id, $allurl = 0) {
	...
	if(!$modelid) return '';
	$db = faeriesCMS_base::load_model('content_model');
...
function upload_key($args) {
	$pc_auth_key = md5(faeriesCMS_base::load_config('system','auth_key').$_SERVER['HTTP_USER_AGENT']);
...
function get_auth_key($prefix,$suffix="") {
	if($prefix=='login'){
		$pc_auth_key = md5(faeriesCMS_base::load_config('system','auth_key').ip());
	}else if($prefix=='email'){
		$pc_auth_key = md5(faeriesCMS_base::load_config('system','auth_key'));
	}else{
		$pc_auth_key = md5(faeriesCMS_base::load_config('system','auth_key').$suffix);
	}
...
function get_pc_version($type='') {
	$version = faeriesCMS_base::load_config('version');
	if($type==1) {
...
function runhook($method) {
	...
	if(!empty($hook_appid)) {
		foreach($hook_appid as $appid => $p) {
			$pluginfilepath = FC_PATH.'plugin'.DIRECTORY_SEPARATOR.$p.DIRECTORY_SEPARATOR.'hook.class.php';
			$getpclass = TRUE;
...
function p_template($plugin = 'content', $template = 'index',$style='default') {
	if(!$style) $style = 'default';
	$template_cache = faeriesCMS_base::load_sys_class('template_cache');
	$compiledtplfile = faeriesCMS_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.'plugin'.DIRECTORY_SEPARATOR.$plugin.DIRECTORY_SEPARATOR.$template.'.php';

	if(!file_exists($compiledtplfile) || (file_exists(FC_PATH.'plugin'.DIRECTORY_SEPARATOR.$plugin.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$template.'.html') && filemtime(FC_PATH.'plugin'.DIRECTORY_SEPARATOR.$plugin.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$template.'.html') > filemtime($compiledtplfile))) {
		$template_cache->template_compile('plugin/'.$plugin, $template, 'default');
	} elseif (!file_exists(FC_PATH.'plugin'.DIRECTORY_SEPARATOR.$plugin.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$template.'.html')) {
...
function get_vid($contentid = 0, $catid = 0, $isspecial = 0) {
	...
	if (!$isspecial) {
		...
		$video_content = faeriesCMS_base::load_model('video_content_model');
		$r = $video_content->get_one(array('contentid'=>$contentid, 'modelid'=>$modelid), 'videoid', '`listorder` ASC');
		$video_store =faeriesCMS_base::load_model('video_store_model');
		return $video_store->get_one(array('videoid'=>$r['videoid']));
	} else {
		$special_content = faeriesCMS_base::load_model('special_content_model');
		$contentid = intval($contentid);
		$video_store =faeriesCMS_base::load_model('video_store_model');
