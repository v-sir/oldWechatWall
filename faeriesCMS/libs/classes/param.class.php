...
class param {
	...
	public function __construct() {
		...
		$this->route_config = faeriesCMS_base::load_config('route', SITE_URL) ? faeriesCMS_base::load_config('route', SITE_URL) : faeriesCMS_base::load_config('route', 'default');
	...
	public static function set_cookie($var, $value = '', $time = 0) {
		...
		$var = faeriesCMS_base::load_config('system','cookie_pre').$var;
		$_COOKIE[$var] = $value;
		if (is_array($value)) {
			foreach($value as $k=>$v) {
				setcookie($var.'['.$k.']', sys_auth($v, 'ENCODE'), $time, faeriesCMS_base::load_config('system','cookie_path'), faeriesCMS_base::load_config('system','cookie_domain'), $s);
			}
		} else {
			setcookie($var, sys_auth($value, 'ENCODE'), $time, faeriesCMS_base::load_config('system','cookie_path'), faeriesCMS_base::load_config('system','cookie_domain'), $s);
		}
	}
