        ...
        <div class="col-md-12 column">
            ...
            <table class="table table-striped table-bordered white-bg">
               ...
               if(isset($_SESSION['loginstatus'])){
                ...
                $tp_id=@$_GET['tp_id'];
                if(isset($tp_id)){
                    $sql="select* from user left join message on user.openid=message.openid  where message.tp_id='$tp_id' order by message.create_at desc  ";
                }
                else{
                    $sql="select* from user left join message on user.openid=message.openid  order by message.create_at desc  ";
                }
                ...
                if(isset($tp_id)){
                    $sql="select* from user left join message on user.openid=message.openid  where message.tp_id='$tp_id' order by message.create_at desc limit $limit ";
                }
                else{
                    $sql="select* from user left join message on user.openid=message.openid  order by message.create_at desc limit $limit ";
                }
                ...
                $db_result = $db_conn->query($sql);
               //$num=$db_result->num_rows;?>
                <form id="batch1" method="post"  action="index.php?m=admin&tpl=wechat&page=<?=$page?>&a=wc_onWallIds&tp_id=<?=$tp_id?>">
                    <form id="batch2" method="post"  action="index.php?m=admin&page=<?=$page?>&a=wc_NotOnWallIds&tp_id=<?=$tp_id?>">
               <? while($info=$db_result->fetch_assoc()){
                ...
                <tr>
                    ...
                    <td>
                        <a href="index.php?m=admin&a=wcMsg_del&id=<?=$info['id']?>&page=<?=$page?>&tp_id=<?=$tp_id?>" class="btn btn-danger btn-xs"><? echo L('delete')?></a>&nbsp;&nbsp;&nbsp;
                        <? if($info['display']==0){
                            ?> <a href="index.php?m=admin&a=wc_onWall&id=<?=$info['id']?>&page=<?=$page?>&tp_id=<?=$tp_id?>" class="btn btn-success btn-xs"><?echo L('do_on')?></a>

                        <?}else{?>
                            <a href="index.php?m=admin&a=wc_NotOnWall&id=<?=$info['id']?>&page=<?=$page?>&tp_id=<?=$tp_id?>" class="btn  btn-danger btn-xs"><?echo L('cancel')?></a>
                ...
                <tr><?
                    ...
                    ?>
                        <a href="index.php?m=admin&c=index&tpl=wechat&page=1&tp_id=<?=$tp_id?>" class="btn btn-success btn-xs" ><? echo L('FirstPage')?></a>
                        <a href="index.php?m=admin&c=index&tpl=wechat&page=<? echo $page-1;?>&tp_id=<?=$tp_id?>" class="btn btn-success btn-xs" ><? echo L('Previous')?></a>

                   <?}
                    if($page<$last){?>
                        <a href="index.php?m=admin&c=index&tpl=wechat&page=<? echo $page+1;?>&tp_id=<?=$tp_id?>" class="btn  btn-danger btn-xs"><? echo L('NextPage')?><a/>
                        <a href="index.php?m=admin&c=index&tpl=wechat&page=<? echo $last?>&tp_id=<?=$tp_id?>" class="btn  btn-danger btn-xs"><? echo L('LastPage')?><a/>
                       <?} ?>
