        ...
        <div class="col-md-12 column">
            ...
            <table class="table table-striped table-bordered white-bg">
                ...
                <tr>
                    ...
                    $tp_id=@$_GET['tp_id'];
                    if(isset($tp_id)){
                        $sql="select* from user left join message on user.openid=message.openid where message.tp_id='$tp_id' && message.display=0  ";
                    }
                    else{
                        $sql="select* from user left join message on user.openid=message.openid   where message.display=0 ";
                    }
                    ...
                    if(isset($tp_id)){
                        $sql="select* from user left join message on user.openid=message.openid where message.tp_id='$tp_id' && message.display=0 order by message.create_at desc limit $limit ";
                    }
                    else{
                        $sql="select* from user left join message on user.openid=message.openid   where message.display=0 order by message.create_at desc limit $limit ";
                    }
                    $db_conn=new mysqli('localhost','root','NIUBSky3!.comr720','new_wechat');
                    if ($db_conn->connect_error) {
                        echo"<script>alert('error:Database connection failed!')</script>";
                    ...
                    $db_conn->query("set names UTF8");
                    $db_result = $db_conn->query($sql);?>
                    <form id="batch1" method="post"  action="index.php?m=admin&tpl=wechat_inactive&a=wc_onWallIds&tp_id=<?=$tp_id?>">
                ...
                <tr><?
                    ...
                    if($page>1) {
                        ...
                        ?>
                        <a href="index.php?m=admin&c=index&tpl=wechat_inactive&page=1&tp_id=<?=$tp_id?>" class="btn btn-success btn-xs" ><? echo L('FirstPage')?></a>
                        <a href="index.php?m=admin&c=index&tpl=wechat_inactive&page=<? echo $page-1;?>&tp_id=<?=$tp_id?>" class="btn btn-success btn-xs" ><? echo L('Previous')?></a>

                    <?}
                    if($page<$last){?>
                    <a href="index.php?m=admin&c=index&tpl=wechat_inactive&page=<? echo $page+1;?>&tp_id=<?=$tp_id?>" class="btn  btn-danger btn-xs"><? echo L('NextPage')?><a/>
                        <a href="index.php?m=admin&c=index&tpl=wechat_inactive&page=<? echo $last?>&tp_id=<?=$tp_id?>" class="btn  btn-danger btn-xs"><? echo L('LastPage')?><a/>
                            <?} ?>
