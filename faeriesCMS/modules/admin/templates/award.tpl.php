        ...
        <div class="col-md-12 column">
            <center><div class="panel panel-info">
                    <div class="panel-heading">
                        <h3 class="panel-title"><? echo L('awardManage')?></h3>
                    </div>
            ...
            <table class="table table-striped table-bordered white-bg">
               ...
               if(isset($_SESSION['loginstatus'])){
                ...
                $tp_id=@$_GET['tp_id'];
                if(isset($tp_id)){
                    $sql="select* from Awards left join user on Awards.openid=user.openid where Awards.tp_id='$tp_id'  ";
                }
                else{
                    $sql="select* from Awards left join user on Awards.openid=user.openid  ";
                }
                ...
                if(isset($tp_id)){
                    $sql="select* from Awards left join user on Awards.openid=user.openid where Awards.tp_id='$tp_id' limit $limit ";
                }
                else{
                    $sql="select* from Awards left join user on Awards.openid=user.openid limit $limit ";
                }
                ...
                <tr>
                    <td><?=$info['id']?></td>
                    <td><?=$info['tp_id']?></td>
                    <td><? $tmpStr = $info['nickname'];
                        echo $this->get_emoji($tmpStr,true); ?></td>
                    <td><img src="<?=$info['headimgurl']?>" width="50px" height="50px"/></td>
                    <td><?=$info['remark']?></td>
                    <td><?=@date('Y-m-d H:i:s',$info['create_at'])?></td>
                    <td>
                        <a href="index.php?m=admin&a=award_del&tp_id=<?=$tp_id?>&id=<?=$info['id']?>&page=<?=$page?>" class="btn btn-danger btn-xs"><? echo L('delete')?></a>&nbsp;&nbsp;&nbsp;

                        <a href="index.php?m=admin&a=award_update&tp_id=<?=$tp_id?>&id=<?=$info['id']?>" class="btn btn-success btn-xs"><?echo L('update')?></a>
                ...
                <tr><?
                    ...
                    if($page>1) {
                        ...
                        ?>
                        <a href="index.php?m=admin&c=index&tpl=award&page=1&tp_id=<?=$tp_id?>" class="btn btn-success btn-xs" ><? echo L('FirstPage')?></a>
                        <a href="index.php?m=admin&c=index&tpl=award&page=<? echo $page-1;?>&tp_id=<?=$tp_id?>" class="btn btn-success btn-xs" ><? echo L('Previous')?></a>

                    <?}
                    if($page<$last){?>
                    <a href="index.php?m=admin&c=index&tpl=award&page=<? echo $page+1;?>&tp_id=<?=$tp_id?>" class="btn  btn-danger btn-xs"><? echo L('NextPage')?><a/>
                        <a href="index.php?m=admin&c=index&tpl=award&page=<? echo $last?>&tp_id=<?=$tp_id?>" class="btn  btn-danger btn-xs"><? echo L('LastPage')?><a/>
                            <?} ?>
